...
runs/jan2022_J20_trade_7h/
  lambda0p1_random01/input.xml
  ...
  lambda0p1_random10/input.xml
                             active ten-start MAP XMLs

results/compact/jan2022_J20_trade_7h/
  map_runs_lambda0p1.csv     ten-start posterior/likelihood/diagnostic table
  best_by_window.csv         highest-posterior row, `lambda0p1_random09`
  matrices/                  manuscript drift matrices for all ten starts;
                             selection, reconstruction, and finite-time
...
The active seven-hour public result is distributed through compact summaries,
the ten MAP XMLs, drift matrices for all ten starts, and detailed matrices for
the plotted highest-posterior start; raw cluster stdout/log files are not
shipped.
...
`lambda0p1_random09` and shows the manuscript-scale drift matrix, including
diagonal entries.
...
Rscript scripts/plot_bitmex_trade_drifts.R \
  ...
  --matrix-csv results/compact/jan2022_J20_trade_7h/matrices/jan2022_J20_trade_7h_lambda0p1_random09_manuscript_drift_matrix.csv \
  --matrix-kind drift \
  --show-diagonal true \
  --layout paper-row \
...
run any active start from the repository root, for example:

```bash
./run_local_beast.sh /path/to/beast.jar \
  applications/bitmex/runs/jan2022_J20_trade_7h/lambda0p1_random09/input.xml
```

Repeat for `lambda0p1_random01` through `lambda0p1_random10` to reproduce the
ten-start MAP battery.
